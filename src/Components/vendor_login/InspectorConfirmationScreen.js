...
export default class InspectorConfirmationScreen extends Component {
	constructor(props) {
		super(props);
		this.state = {
			details: [],
	...
	onCnfm(item) {
		...
		this.setState(
			{
				siteofferQty: item.offerQty.toString(),
			...
			responseJson => {
				...
				if (responseJson.status == 'SUCCESS') {
					...
					if (this.state.qtystatus == 'Y') {
						...
						APIManager.workorderMatQtyAdd(
							this.state.pdiofferAiId,
							this.state.siteofferQty,
							responseJson => {
								console.log(JSON.stringify(responseJson));
								if (responseJson.status == 'SUCCESS') {
									this.getInspectorConfirmationDetails();
							...
							<FlatList
								...
								renderItem={({ item, index }) => (
									<View
										...
										style={[
											styles.cardStyle,
											{ borderLeftColor: item.vendorConfirmStatus == '0' ? '#61B865' : '#E63E3A' }
										]}
									>
