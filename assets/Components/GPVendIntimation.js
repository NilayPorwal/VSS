...
global.GPVendIntimation;    
type Props = {};  
export default class GPVendIntimation extends Component<Props> {
  ...
  componentDidMount(){
    ...
    async getInspId(){  
		 await AsyncStorage.getItem('InspId').then((value) => 
			this.setState({inspId:value},()=>{this.getGPNomination();} )	
		 )         
	 } 
  ...
  getGPNomination(){  
   ...
   APIManager.getGPNomination(this.state.inspId,
    (responseJson)=> { 
	...
	redirectTo(item){
     this.props.navigation.push('GPConfirmInspectionScreen', {
     	vendorInfo:item,
     	onGoBack:()=>{this.getInspId();}
     })
  ...
  render() {    
    ...
    return (
      ...
      <TouchableOpacity onPress={()=>this.props.navigation.goBack()}>
       ...
       <Text style={{fontSize:15, fontFamily:'GoogleSans-Medium', color:'black', paddingTop:15, paddingLeft:10}}>Nomination Confirmation to Vendor</Text>
      ...
      {(this.state.vendorInfo.length > 0)?
        <FlatList
			...
			<View style={{flexDirection:'row'}}>
			  <View style={{width:'50%'}}>
			    <Text style={{color:'#000000', fontFamily:'GoogleSans-Medium', fontSize:15}}>Vendor Name</Text>
			    <Text style={{color:'#000000',  fontSize:13}}>{item.vendorFirmName}</Text>
			  </View> 
			   
			  <View  style={{width:'50%'}}>
			    <Text  style={{color:'#000000', fontFamily:'GoogleSans-Medium', fontSize:15, textAlign:'right'}}>Work Order Address</Text>
			    <Text  style={{color:'#000000', fontSize:13, textAlign:'right'}}>{item.vendorWorkAddress}</Text>
			  </View> 
			</View>

			<View style={{flexDirection:'row', marginTop:10}}>
			  <View style={{width:'50%'}}>
			    <Text style={{color:'#000000', fontFamily:'GoogleSans-Medium', fontSize:15}}>Tender No.</Text>
			    <Text style={{color:'#000000',  fontSize:13}}>{item.tenderNumber}</Text>
			  </View> 
			  
			  <View  style={{width:'50%'}}>
			    <Text  style={{color:'#000000', fontFamily:'GoogleSans-Medium', fontSize:15, textAlign:'right'}}>Work Order No.</Text>
			    <Text  style={{color:'#000000', fontSize:13, textAlign:'right'}}>{item.woGpNumber}</Text>
			  </View> 
			</View>
  
			<View style={{flexDirection:'row', marginTop:10}}>
			  <View style={{width:'50%'}}>
			    <Text style={{color:'#000000', fontFamily:'GoogleSans-Medium', fontSize:15}}>Work Order Id</Text>
			    <Text style={{color:'#000000',  fontSize:13}}>{item.woCustomUnqId}</Text>
			  </View> 
			  
			  <View  style={{width:'50%'}}>
			    <Text  style={{color:'#000000', fontFamily:'GoogleSans-Medium', fontSize:15, textAlign:'right'}}>Work Order Date</Text>
			    <Text  style={{color:'#000000', fontSize:13, textAlign:'right'}}>{item.nominatioDateTime}</Text>
			  </View> 
			</View>  
			...
			<View style={{flexDirection:'row', marginTop:15}}>
			  ...
			  <View  style={{width:'50%'}}>
			      <TouchableOpacity onPress={()=>{this.redirectTo(item)}}   style={{borderRadius: 5, backgroundColor: '#418bca'}}>
					<Text style={{fontSize:15, color:'#ffffff', fontFamily:'GoogleSans-Medium', paddingVertical:10, textAlign:'center', elevation:5}}>Intimate Vendor</Text>
				  </TouchableOpacity> 
			  </View> 
			</View>     
