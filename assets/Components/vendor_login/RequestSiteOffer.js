...
type Props = {};  
export default class RequestSiteOffer extends Component<Props> {
  ...
  async getVendorId(){  
		 await AsyncStorage.getItem('InspId').then((value) => 
			this.setState({vndId:value},()=>{this.getOfferDetails()} )	
		 )         
	 }

   
 getOfferDetails(){  
        APIManager.getOfferDetails(this.state.vndId, 
        (responseJson)=> {    
  ...
  render() {    
    ...
    return (
     ...
     <View style={{width:'20%'}} style={{flexDirection:'row'}}>
      ...
      <View style={{width:'80%'}}> 
       <Text style={{fontSize:18, fontFamily:'GoogleSans-Medium', color:'black', paddingTop:15, textAlign:'center' }}>List of Submitted Offer</Text>
      </View>  
     </View> 
       ...
       {(this.state.woDetails.length > 0)?
			...
			<View style={[styles.cardStyle,{ borderLeftColor:(item.siteOfferUploadStatus == 'I')?"#61B865":"#E63E3A"}]}>
	        ...
	        {(item.siteOfferUploadStatus == 'I')?
             <View style={{flexDirection:'row', justifyContent:'space-between', marginVertical:10}}>
				  ...
				  <TouchableOpacity  onPress={()=>this.redirectTo(item)}  style={{borderRadius: 5, backgroundColor: '#418bca'}}>
					<Text style={{fontSize:15, color:'#ffffff', fontFamily:'GoogleSans-Medium', paddingVertical:10, paddingHorizontal:20, elevation:5}}>Request Site Schedule</Text>
				  </TouchableOpacity> 
             </View>:null } 
	        ...
	        {(item.siteOfferUploadStatus == 'N')? 
             <View style={{flexDirection:'row', justifyContent:'space-between', marginVertical:10}}>
				  ...
				  <TouchableOpacity  onPress={()=>this.redirectTo(item)}  style={{borderRadius: 5, backgroundColor: '#ff7f00'}}>
					<Text style={{fontSize:15, color:'#ffffff', fontFamily:'GoogleSans-Medium', paddingVertical:10, paddingHorizontal:20, elevation:5}}>Upload Site Offer</Text>
				  </TouchableOpacity> 
             </View>:null } 
