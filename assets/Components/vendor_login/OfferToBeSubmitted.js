...
import {Platform, StyleSheet, Text, View, Image, TouchableOpacity,Alert,
		...
		ActivityIndicator, AsyncStorage, ScrollView, FlatList, CheckBox, ToastAndroid} from 'react-native';
...
type Props = {};  
export default class OfferToBeSubmitted extends Component<Props> {
  ...
  getofferDetails(){
       const materialSubCatAiId = (typeof this.state.woDetails.materialSubCatAiId != "undefined")?this.state.woDetails.materialSubCatAiId:this.state.woDetails.materialSubcatAiId;
	   APIManager.getofferDetails(this.state.woDetails.woAiId, this.state.woDetails.materialAiId, materialSubCatAiId, this.state.type,
        (responseJson)=> {
   ...
   onSubmitOffer(){
       if (this.state.siteofferQty == null || this.state.siteofferQty == ''){
	   ...
	   else if (this.state.isHeadSchedule == "N" && this.state.siteofferQty > this.state.remainingSchQty && this.state.type == "PDI"){
		   ...
		   APIManager.onSubmitOffer(Details, this.state.type,
			   (responseJson) => {
				   ...
				   if (responseJson.status == 'SUCCESS') {
                     if(this.state.stageInspectionStatus == "Y"){
					  ...
					  if(this.state.imageData.length == 0){
                        this.setState({disabled:false, isRefreshing: false, modalVisible1: false,siteofferQty:null, imageData:[], images:[], packStartSrno:null, packEndSrno:null})
   ...
   updateStageInspection(pdiOfferAiId){
	...
	console.log(JSON.stringify(Details)) 
	APIManager.updateStageInspection(Details,
		(responseJson) => {
			console.log(JSON.stringify(responseJson));  
			if (responseJson.status == 'SUCCESS') {
   ...
   uploadImages(pdiOfferAiId){
	...
	remainingDetails() {
		...
		const woMatMapAiId = (typeof this.state.woDetails.woMatMapAiId != "undefined")?this.state.woDetails.woMatMapAiId:this.state.woDetails.woMaMapAiId;
		...
		APIManager.getRemainingDetails(this.state.woDetails.woAiId, woMatMapAiId,uri,
			(responseJson) => {
				console.log(JSON.stringify(responseJson));
	...
	getRemainingScheduleQty(item, type) {
		//this.setState({ isRefreshing: true })
	this.setState({
		 deliverySchAiId: item.delivery_sch_id, offerSupplyDat: item.delivery_sch_date, isHeadSchedule: item.isHeadSchedule, stageInspectionStatus:item.stageInspectionStatus
	})	
	...
	APIManager.getRemainingScheduleQty(item.delivery_sch_id, flag,
			(responseJson) => {
				//console.log(JSON.stringify(responseJson));
				//alert(JSON.stringify(responseJson.data.balanceQty));
   ...
   onOfferReq(item){
	...
	//     return true;
	this.setState({
		siteofferQty: item.delivery_sch_qty, deliverySchAiId: item.delivery_sch_id, offerSupplyDat: item.delivery_sch_date
	})
	this.setState({
		modalVisible1: true, 
	})	 
  ...
  render() {    
    ...
    return (
     ...
     <View style={{width:'20%'}} style={{flexDirection:'row'}}>
      ...
      <View style={{width:'80%'}}> 
       <Text style={{fontSize:18, fontFamily:'GoogleSans-Medium', color:'black', paddingTop:15, textAlign:'center' }}>Offer To Be Submitted</Text>
      </View>  
     </View> 
        ...
        <View style={styles.detailsView}>  
		 ...
		 <View style={{flexDirection:'row', justifyContent:'space-between'}}>
		  <View style={{width:'50%'}}>
		   ...
		   <Text style={{color:'black', flex:1,  flexWrap: 'wrap'}}>{this.state.woDetails.woTenderNumber}</Text>    
		  </View>
		  
		  <View style={{width:'50%'}}>
    	   <Text style={{color:'black', fontFamily:'GoogleSans-Medium', textAlign:'right'}}>Work Order No.</Text>
		   <Text style={{color:'black', textAlign:'right', flex:1,  flexWrap: 'wrap'}}>{this.state.woDetails.woNumber}</Text>
		  </View>
		 </View>     
		 ...
		 <View style={{flexDirection:'row', justifyContent:'space-between', marginTop:20}}>
		  ...
		  <View style={{width:'50%'}}>
    	   <Text style={{color:'black', fontFamily:'GoogleSans-Medium', textAlign:'right'}}>Material Category</Text>
		   <Text style={{color:'black', textAlign:'right', flex:1,  flexWrap: 'wrap'}}>{this.state.woDetails.materialName }</Text>
		  </View> 
		 </View>  
		 ...
		 <View style={{flexDirection:'row', justifyContent:'space-between', marginTop:20}}>
		  <View style={{width:'50%'}}>
		   ...
		   <Text style={{color:'black', flex:1,  flexWrap: 'wrap'}}>{this.state.woDetails.materialScName}</Text>
		  </View>
		      
		  <View style={{width:'50%'}}>
    	   <Text style={{color:'black', fontFamily:'GoogleSans-Medium', textAlign:'right'}}>Total Quantity</Text>
		   <Text style={{color:'black', textAlign:'right', flex:1,  flexWrap: 'wrap'}}>{this.state.woDetails.materialQty }</Text>
		  </View> 
		 </View>   
		</View>
        ...
        <FlatList
			...
			<View style={[styles.cardStyle,{ borderLeftColor:(item.dispOfferOpt == 'Y')?"#61B865":"#E63E3A"}]}>
			
			<View style={{flexDirection:'row'}}>
			  <View style={{width:'50%'}}>
			    <Text style={{color:'#000000', fontFamily:'GoogleSans-Medium', fontSize:15}}>Supply Schedule No.</Text>
			    <Text style={{color:'#000000',  fontSize:13}}>{index+1}</Text>
			  </View> 
			  
			  <View  style={{width:'50%'}}>
			    <Text  style={{color:'#000000', fontFamily:'GoogleSans-Medium', fontSize:15, textAlign:'right'}}>Days From Work Order</Text>
			    <Text  style={{color:'#000000', fontSize:13, textAlign:'right'}}>{item.delivery_sch_days}</Text>
			  </View> 
			</View>

			<View style={{flexDirection:'row', marginTop:10}}>
			  <View style={{width:'50%'}}>
			    <Text style={{color:'#000000', fontFamily:'GoogleSans-Medium', fontSize:15}}>Expected Date of Supply</Text>
			    <Text style={{color:'#000000',  fontSize:13}}>{item.delivery_sch_date}</Text>
			  </View> 
			  
			  <View  style={{width:'50%'}}>
			    <Text  style={{color:'#000000', fontFamily:'GoogleSans-Medium', fontSize:15, textAlign:'right'}}>Expected Suply Quantity</Text>
			    <Text  style={{color:'#000000', fontSize:13, textAlign:'right'}}>{item.delivery_sch_qty}</Text>
			  </View> 
			</View> 
           ...
           {(item.dispOfferOpt == 'Y' && item.diffStatus == "N")?
             <View style={{flexDirection:'row', justifyContent:'space-between', marginVertical:10}}>
				  ...
				  <TouchableOpacity onPress={()=>this.getRemainingScheduleQty(item, 0)}   style={{borderRadius: 5, backgroundColor: '#418bca'}}>
					<Text style={{fontSize:15, color:'#ffffff', fontFamily:'GoogleSans-Medium', paddingVertical:10, paddingHorizontal:20, elevation:5}}>Offer Request</Text>
				  </TouchableOpacity>:
				  <View style={{flexDirection:"row"}}>
					<TouchableOpacity onPress={()=>this.getRemainingScheduleQty(item, 0)}   style={{borderRadius: 5, backgroundColor: '#418bca'}}>
						<Text style={{fontSize:15, color:'#ffffff', fontFamily:'GoogleSans-Medium', paddingVertical:10, paddingHorizontal:20, elevation:5}}>Offer Request</Text>
					</TouchableOpacity>
					<TouchableOpacity onPress={()=>this.getRemainingScheduleQty(item, 1)}   style={{borderRadius: 5, backgroundColor: '#418bca', marginLeft:10}}>
						<Text style={{fontSize:15, color:'#ffffff', fontFamily:'GoogleSans-Medium', paddingVertical:10, paddingHorizontal:20, elevation:5}}>Stage Inspection</Text>
					</TouchableOpacity>
				  </View>}
             </View>:null } 
		 ...
		 <Modal  
          ...
          onRequestClose={() => {
            ...
            <View onPress={()=>this.setState({modalVisible1:false})} style={{ flex: 1,  justifyContent: 'center', alignItems: 'center',backgroundColor:'#00000080'}}>
			...
			<View style={{ width: '80%', backgroundColor:'#ffffff',padding:15,borderRadius:10}}>
			   ...
			   <View style={{ marginTop:15}}>
			    ...
			    <View style={{}}>   
				 ...
				 {(this.state.type == "PDI" && this.state.isHeadSchedule == "N")?
				  <Text style={{color:"#000000"}}>Schedule Remaining Quantity: {this.state.remainingSchQty}</Text>:null 
				 } 
				  <Text style={{color:"#000000"}}>Total Material Schedule Remaining Quantity: {this.state.blnsQty}</Text> 
				 </View>
		        </View>
               ...
               <View style={{ flexDirection: 'row', marginTop:15, justifyContent:"space-between"}}>
                <Text style={{color:'#000000', fontFamily:'GoogleSans-Medium', fontSize:15}}>Readiness Date : </Text>
		        <TouchableOpacity onPress={()=>{this.setState({isDatePickerVisible:true})}}  style={{ flexDirection: 'row', borderWidth:1, borderColor:'#418bca', padding:5}}>
               ...
               <View style={{flexDirection:'row', justifyContent:'space-between', marginVertical:20}}>
				 <TouchableOpacity disabled={this.state.disabled} onPress={()=>this.onSubmitOffer()} style={{borderRadius: 5, backgroundColor: '#3CB043', width:100, alignItems:"center"}}>
				  ...
				  <TouchableOpacity  onPress={()=>this.setState({modalVisible1:false, remainingSchQty:null})}  style={{borderRadius: 5, backgroundColor: 'red',  width:100, alignItems:"center"}}>
               ...
               <TouchableOpacity onPress={()=>this.setState({modalVisible1:false})} style={{position:'absolute', top:0, right:5}}>
		...
		<Modal  
          ...
          onRequestClose={() => {
           ...
           <View style={{ width: 300, backgroundColor:'#ffffff',padding:10,borderRadius:10}}>
			 <Text style={{fontSize:18, fontFamily:'GoogleSans-Medium', textAlign:'center', color:'#ff7f00'}}>Offer Request</Text>
			 <TouchableOpacity onPress={()=>{this.setState({ withPackingList: !this.state.withPackingList })}} style={{ flexDirection: 'row', marginTop:15 }}>
			  <CheckBox
            ...
            <View> 	
			  <View style={{flexDirection:'row', marginTop:20}}>
			   <Text style={{color:'black', textAlign:'center', width:'40%', fontFamily:'GoogleSans-Medium', paddingTop:10}}>Start Serial No. :</Text>
			 ...
			 <View style={{flexDirection:'row', marginTop:20}}>
			   <Text style={{color:'black', textAlign:'center', width:'40%', fontFamily:'GoogleSans-Medium', paddingTop:10}}>End Serial No. :</Text>
              ...
              <TouchableOpacity onPress={()=>this.onSubmitOffer()} style={{borderRadius: 5, backgroundColor: '#418bca', marginTop:15}}> 
			   <Text style={{fontSize:15, color:'#ffffff', fontFamily:'GoogleSans-Medium', paddingVertical:10, textAlign:'center',  elevation:5}}>Submit</Text>
			  </TouchableOpacity> 
