...
import { Image, ImageBackground, TouchableOpacity, Dimensions, Platform,FlatList,TextInput,
         PermissionsAndroid, StyleSheet, Text, View, AsyncStorage, BackHandler, Modal, Picker, ActionSheetIOS } from 'react-native';
...
export class AddressConfirmation extends React.Component {
  ...
  componentDidMount(){   
   ...
   vendorAddressList(){  
         APIManager.vendorAddressList((responseJson)=> { 
	...
	getGeometry(address,success, failure){ 
		...
		fetch("https://maps.googleapis.com/maps/api/geocode/json?address="+address+"&key=AIzaSyCrsfCq7GRSpXXuvhxOnwu36u2wKdwqOos")
...
async onEditPress(item){    
   // await  this.getCityList(item.workAddressState)
 	await  global.AddressConfirmation.setState({area:item.vendorWorkAddress, 
 ...
 getCityList(stateCode){
 	 cityNameList = []
      APIManager.getCityList(stateCode,(responseJson)=> { 
	        console.log(JSON.stringify(responseJson));  
	        if (responseJson.status == 'SUCCESS'){ 
 ...
 getStateList(){
      APIManager.getStateList((responseJson)=> { 
	       console.log(JSON.stringify(responseJson));  
	        if (responseJson.status == 'SUCCESS'){ 
 ...
 onSaveWorkAddress(){ 
 	...
 	this.getGeometry(this.state.area,  (response)=> { 
       ...
       const insertData = [
        ...
        APIManager.onSaveWorkAddress(insertData, (responseJson)=> { 
   ...
   stateList(){
	 	 ActionSheetIOS.showActionSheetWithOptions({
               ...
               if(buttonIndex != 0){
                   this.setState({stateCode: this.state.stateList[buttonIndex-1].stateAiId, stateName: this.state.stateList[buttonIndex-1].stateName}, ()=>{
	 	 ...
	 	 ActionSheetIOS.showActionSheetWithOptions({
                options: ["Cancel", ...cityNameList],
                 cancelButtonIndex: 0,
            	  ...
            	  if(buttonIndex != 0){
                   this.setState({cityCode: this.state.cityList[buttonIndex-1].distAiId, cityName: this.state.cityList[buttonIndex-1].distName})
                  }
          })
	 }
    ...
    return (
	  <ImageBackground source={require('../../Images/background.png')} style={{width: '100%', height: '100%'}}>
       <View style={styles.mainContainer}>
        <Text style={{fontSize:18, fontFamily:'GoogleSans-Medium', marginTop:25, color:'black',  letterSpacing: 3, paddingHorizontal:10, textAlign:'center'}} >Work Address</Text>
          {(this.state.addressInfo.length > 0)?  
		   ...
		   />:
		   <Text style={{fontFamily:'GoogleSans-Medium', textAlign:"center", marginTop:20, fontSize:20}}>No data found</Text>
		}
          ...
          onRequestClose={() => {
           ...
           <View  style={{ flex: 1,  justifyContent: 'center', alignItems: 'center',backgroundColor:'#00000080'}}>
			...
			<View style={{marginTop:10}}> 
			  <Text style={{color:"#000000", fontSize:15, fontFamily:'GoogleSans-Medium', textAlign:"center"}}>Edit Address</Text>   
			  ...
			  />   
                ...
                <View style={{backgroundColor:"#d3d3d3"}}>
				    ...
				    <Picker 
					  ...
					  mode='dropdown'
					  onValueChange={(itemValue) => this.setState({stateCode:itemValue},()=>this.getCityList(this.state.stateCode))}>
						  {state} 
					</Picker>}
				</View>
			   ...
			   <TouchableOpacity onPress={()=> this.onSaveWorkAddress()} style={{borderRadius: 5, backgroundColor: '#418bca', marginTop:15}}>
				<Text style={{fontSize:15, color:'#ffffff', paddingVertical:12, textAlign:'center'}}>Save</Text>
			   </TouchableOpacity> 
            
                
			   <TouchableOpacity onPress={()=> this.setState({ModalVisible:false})} style={{borderRadius: 5, backgroundColor: '#418bca', marginTop:15}}>
				<Text style={{fontSize:15, color:'#ffffff', paddingVertical:12, textAlign:'center'}}>Cancel</Text>
			   </TouchableOpacity> 
