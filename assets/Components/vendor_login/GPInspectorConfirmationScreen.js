...
type Props = {};  
export default class GPInspectorConfirmationScreen extends Component<Props> {
  ...
  async getInspId(){  
		 await AsyncStorage.getItem('InspId').then((value) => 
			this.setState({vendId:value},()=>{this.getGPInspectorConfirmationDetails()} )	
		 )         
	 } 
  ...
  getGPInspectorConfirmationDetails(){
      ...
      APIManager.getGPInspectorConfirmationDetails(this.state.vendId, 
        (responseJson)=> { 
  ...
  onCnfm(item){ 
	  ...
	  this.setState({item:item}, ()=>{ this.setState({modalVisible:true})})
   }    
  ...
  onConfirm(){
    ...
    const  Details = {
        ...
        APIManager.GPVendorConfirmation(Details,
        (responseJson)=> {
        	this.setState({isRefreshing:false})	
	        console.log(JSON.stringify(responseJson));
	        if (responseJson.status == 'SUCCESS'){
      ...
      {(this.state.details.length > 0)?
        <FlatList    
			...
			<View style={{flexDirection:'row'}}>
			 <View  style={{width:'50%'}}>
			    <Text  style={{color:'#000000', fontFamily:'GoogleSans-Medium', fontSize:15}}>Inspector Name</Text>
			    <Text  style={{color:'#000000', fontSize:13}}>{item.inspectorName}</Text>
			  </View> 
			  <View style={{width:'50%'}}>
			    <Text style={{color:'#000000', fontFamily:'GoogleSans-Medium', fontSize:15, textAlign:'right'}}>Contact Number</Text>
		  ...
		  <View onPress={()=>this.closeModal()} style={{ flex: 1,  justifyContent: 'center', alignItems: 'center',backgroundColor:'#00000080'}}>			  
             <View style={{ width: '80%', backgroundColor:'#ffffff',padding:15,borderRadius:10}}>
               ...
               <View style={{flexDirection:'row', justifyContent:'space-between', marginVertical:20}}>
				 <TouchableOpacity onPress={()=>this.onConfirm()} style={{borderRadius: 5, backgroundColor: '#3CB043',}}>
