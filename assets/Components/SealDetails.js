...
import {
	Platform, StyleSheet, Text, View, Image, TouchableOpacity, Alert, KeyboardAvoidingView,
	ScrollView, ActivityIndicator, ImageBackground, BackHandler, ActionSheetIOS,
	AsyncStorage, Picker, TextInput, FlatList, Modal, Dimensions, Linking, CheckBox
} from 'react-native';
...
const packageType = []
const sealType = []
export default class SealDetails extends Component<Props> {
	...
	onSave() {
		...
		db.transaction((tx) => {
			...
			tx.executeSql('CREATE TABLE IF NOT EXISTS sealDetails (id integer primary key,nominationAiId integer, sealTypeName text, sealTypeAiId integer, packageNo integer, packageTypeAiId integer, matSeriallDetails integer , sealNo integer, sealDetail integer, sealStartNo integer, sealEndNo integer )');
			tx.executeSql('INSERT INTO sealDetails (nominationAiId, sealTypeName , sealTypeAiId , packageNo , packageTypeAiId , matSeriallDetails  , sealNo , sealDetail, sealStartNo, sealEndNo ) VALUES (?, ?, ?, ?,?,?,?, ?, ?, ?)', [this.state.siteOfferDetails.nominationAiId, this.state.sealTypeName, this.state.sealTypeAiId, this.state.packageNo, this.state.packageTypeAiId, this.state.matSeriallDetails, this.state.sealNo, this.state.sealDetail, this.state.sealStartNo, this.state.sealEndNo]);
			this.setState({ sealTypeName: null, sealTypeAiId: null, packageNo: null, packageTypeAiId: null, matSeriallDetails: null, sealNo: null, sealDetail: null, sealStartNo: null, sealEndNo: null, formVisible: false, sealModalVisible: false }, () => this.getSealDetails());
	...
	getSealDetails() {
		...
		db.transaction((tx) => {
			tx.executeSql('SELECT * FROM sealDetails ORDER BY id desc', [], (tx, results) => {
				...
				for (let i = 0; i < len; i++) {
					...
					if (row.nominationAiId == this.state.siteOfferDetails.nominationAiId) {
						this.setState({
							...
							}], formVisible: false, isRefreshing: false
						})
					}
				}
	...
	upLoad() {
		...
		APIManager.uploadSealDetails(Details,
			(responseJson) => {
				//	alert(JSON.stringify(responseJson)); 
				if (responseJson.status == 'SUCCESS') {
	...
	uploadTestStatus() {
		...
		APIManager.uploadTestStatus(Details,
			(responseJson) => {
				...
				if (responseJson.status == 'SUCCESS') {
					this.setState({ isloading: false, itemConsumeModel: false, pdfLink: responseJson.data.file }, () => { this.downloadPDF() })
				}
				else {
	...
	packageType() {
		ActionSheetIOS.showActionSheetWithOptions({
			...
			(buttonIndex) => {
				this.setState({ packageTypeAiId: this.state.packageDetails[buttonIndex].packageTypeAiId, packageTypeName: this.state.packageDetails[buttonIndex].packageType })
			})
	}
				...
				<ScrollView>
					...
					<KeyboardAvoidingView style={{ flex: 1 }} behavior="padding" enabled>
						...
						<View style={{ width: '20%' }} style={{ flexDirection: 'row' }}>
							...
							<View style={{ width: '80%' }}>
								<Text style={{ fontSize: 18, fontFamily: 'GoogleSans-Medium', color: 'black', paddingTop: 15, textAlign: 'center' }}>Seal Details</Text>
							</View>
						</View>
								...
								renderItem={({ item, index }) =>
									...
									<View style={{ margin: 15, borderRadius: 5, elevation: 8, backgroundColor: '#ffffff', padding: 15 }}>
										...
										<View style={{ flexDirection: 'row' }}>
											<View style={{ width: '50%' }}>
												<Text style={{ color: '#000000', fontFamily: 'GoogleSans-Medium', fontSize: 15 }}>Package No. :</Text>
												<Text style={{ color: '#000000', fontSize: 13 }}>{item.packageNo}</Text>
											</View>

											<View style={{ width: '50%' }}>
												<Text style={{ color: '#000000', fontFamily: 'GoogleSans-Medium', fontSize: 15, textAlign: 'right' }}>Package Type</Text>
												<Text style={{ color: '#000000', fontSize: 13, textAlign: 'right' }}>{item.packageTypeAiId}</Text>
											</View>
										</View>

										<View style={{ flexDirection: 'row', marginTop: 10 }}>
											<View style={{ width: '50%' }}>
												<Text style={{ color: '#000000', fontFamily: 'GoogleSans-Medium', fontSize: 15 }}>Material Serial Details/Package</Text>
												<Text style={{ color: '#000000', fontSize: 13 }}>{item.matSeriallDetails}</Text>
											</View>

											<View style={{ width: '50%' }}>
												<Text style={{ color: '#000000', fontFamily: 'GoogleSans-Medium', fontSize: 15, textAlign: 'right' }}>Seal Type</Text>
										...
										<View style={{ flexDirection: 'row', marginTop: 10 }}>
											<View style={{ width: '50%' }}>
												<Text style={{ color: '#000000', fontFamily: 'GoogleSans-Medium', fontSize: 15 }}>No of Seals</Text>
												<Text style={{ color: '#000000', fontSize: 13 }}>{item.sealNo}</Text>
											</View>

											<View style={{ width: '50%' }}>
												<Text style={{ color: '#000000', fontFamily: 'GoogleSans-Medium', fontSize: 15, textAlign: 'right' }}>Seal Details</Text>
										...
										<View style={{ flexDirection: 'row', marginTop: 10 }}>
											<View style={{ width: '50%' }}>
												<Text style={{ color: '#000000', fontFamily: 'GoogleSans-Medium', fontSize: 15 }}>Seal Start No</Text>
												<Text style={{ color: '#000000', fontSize: 13 }}>{item.sealStartNo}</Text>
											</View>

											<View style={{ width: '50%' }}>
												<Text style={{ color: '#000000', fontFamily: 'GoogleSans-Medium', fontSize: 15, textAlign: 'right' }}>Seal End No</Text>
						...
						{(this.state.formVisible == true) ?
							<View style={{ borderRadius: 5, backgroundColor: '#ffffff', padding: 15, elevation: 8, margin: 15 }}>
								<View style={{ flexDirection: 'row', marginTop: 10 }}>
									<Text style={{ color: 'black', textAlign: 'center', width: '40%', fontFamily: 'GoogleSans-Medium', paddingTop: 10 }}>Package No. :</Text>
								...
								{
									(Platform.OS === 'ios') ?
										<TouchableOpacity style={{ height: 40, padding: 10, borderWidth: 1, borderColor: '#ff7f00', marginTop: 15 }} onPress={() => { this.packageType() }}>
								...
								<View style={{ flexDirection: 'row', marginTop: 15 }}>
									<Text style={{ color: 'black', textAlign: 'center', width: '40%', fontFamily: 'GoogleSans-Medium', paddingTop: 10 }}>Material Serial Details/Package :</Text>
								...
								{
									(Platform.OS === 'ios') ?
										<TouchableOpacity style={{ height: 40, padding: 10, borderWidth: 1, borderColor: '#ff7f00', marginTop: 15 }} onPress={() => { this.sealType() }}>
										...
										<View style={{ borderWidth: 1, borderColor: '#ff7f00', marginTop: 20 }}>
											<Picker
												...
												style={{ height: 40, width: '100%', }}
												mode='dropdown'
												onValueChange={(itemValue, itemIndex) => this.setState({ sealTypeAiId: itemIndex, sealTypeName: itemValue })}>
												{pickerItems}
											</Picker>
										</View>
								}
								...
								<View style={{ flexDirection: 'row', marginTop: 15 }}>
									<Text style={{ color: 'black', textAlign: 'center', width: '40%', fontFamily: 'GoogleSans-Medium', paddingTop: 10 }}>No of Seals :</Text>
								...
								<View style={{ flexDirection: 'row', marginTop: 20 }}>
									<Text style={{ color: 'black', textAlign: 'center', width: '40%', fontFamily: 'GoogleSans-Medium', paddingTop: 10 }}>Seal Start No. :</Text>
								...
								<View style={{ flexDirection: 'row', marginTop: 20 }}>
									<Text style={{ color: 'black', textAlign: 'center', width: '40%', fontFamily: 'GoogleSans-Medium', paddingTop: 10 }}>Seal End No. :</Text>
								...
								<View style={{ flexDirection: 'row', marginTop: 15 }}>
									<Text style={{ color: 'black', textAlign: 'center', width: '40%', fontFamily: 'GoogleSans-Medium', paddingTop: 10 }}>Seal Details :</Text>
						...
						<View style={{ flexDirection: 'row' }}>
							...
							<View style={{ alignItems: 'center', width: '50%' }}>
								{(this.state.sealData.length > 0) ?
									<TouchableOpacity onPress={() => this.setState({ sealModalVisible: true })} style={{ borderRadius: 5, backgroundColor: '#418bca', marginVertical: 20, paddingVertical: 10, paddingHorizontal: 20 }}>
							...
							<View style={{ alignItems: 'center', width: '50%' }}>
								{(this.state.sealData.length > 0) ?
									<TouchableOpacity onPress={() => this.onSubmit()} style={{ borderRadius: 5, backgroundColor: '#418bca', marginVertical: 20, paddingVertical: 10, paddingHorizontal: 20 }}>
						...
						<Modal
							...
							<View onPress={() => this.setState({ sealModalVisible: false })} style={{ flex: 1, alignItems: 'center', backgroundColor: '#00000080' }}>
								{(this.state.isRefreshing == false) ?
									<ScrollView showsVerticalScrollIndicator={false}>
										<View style={{ width: width * 0.9, backgroundColor: '#ffffff', borderRadius: 10, padding: 15, marginTop: 20 }}>

											<Text style={{ fontSize: 20, fontFamily: 'GoogleSans-Medium', marginTop: 10, color: '#ff7f00', textAlign: 'center' }}>Seal Details</Text>
											...
											<TouchableOpacity onPress={() => this.setState({ sealModalVisible: false })} style={{ position: 'absolute', top: 0, right: 5 }}>
						...
						<Modal
							...
							<View onPress={() => this.setState({ itemConsumeModel: false, })} style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: '#00000080' }}>


								<View style={{ width: '80%', backgroundColor: '#ffffff', padding: 15, borderRadius: 10, justifyContent: 'center' }}>
									...
									{
										(this.state.isloading == true) ? <ActivityIndicator size="large" color="orange" /> :
											<TouchableOpacity onPress={() => (this.state.check == 0) ? this.setState({ itemConsumeModel: false }) : this.uploadTestStatus()} style={{ borderRadius: 5, backgroundColor: '#ff7f00', marginTop: 15 }}>
												<Text style={{ fontSize: 15, color: '#ffffff', paddingVertical: 12, textAlign: 'center' }}>Submit</Text>
											</TouchableOpacity>
									}
								</View>
							</View>
...
const styles = StyleSheet.create({
	...
	tableContent: {
		flexDirection: 'row', borderBottomColor: 'lightgrey', borderBottomWidth: 1, backgroundColor: '#ffffff'
	},
	tableContentText: {
		color: 'black', borderRightColor: 'lightgrey', textAlign: 'center', borderRightWidth: 1, fontSize: 12, paddingVertical: 10, paddingHorizontal: 5
	},
	button: {
		borderRadius: 5, backgroundColor: '#ff7f00', marginVertical: 20, paddingVertical: 10, paddingHorizontal: 20
	},
	buttonText: {
