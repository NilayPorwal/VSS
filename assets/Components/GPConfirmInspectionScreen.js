...
import {Platform, StyleSheet, Text, View,TextInput, Image, TouchableOpacity,Alert,
        BackHandler, Modal, TouchableWithoutFeedback,ImageBackground, ActionSheetIOS,
		ActivityIndicator, AsyncStorage, ScrollView, FlatList, Picker, KeyboardAvoidingView, ToastAndroid} from 'react-native';
...
global.GPConfirmInspectionScreen;    
type Props = {};  
export default class GPConfirmInspectionScreen extends Component<Props> {
       ...
       super(props);     
		  ...
		  global.GPConfirmInspectionScreen = this;
	   } 
  ...
  onSubmit(){
    if(this.state.Date == null || this.state.Time == null||this.state.mode === 0 || this.state.location == null){
     ...
     this.setState({isRefreshing:true})
	   const  Details = {
            "nominationGpAiId" :this.state.vendorInfo.nominationGpAiId.toString(),
			"woGpAiId" :this.state.vendorInfo.woGpAiId.toString(),
        ...
        APIManager.onGPConfirmInsp(Details, 
        (responseJson)=> {
    ...
    return (
	  ...
	  <KeyboardAvoidingView style={styles.container} behavior="padding" enabled>
        ...
        <Text style={{fontSize:18, fontFamily:'GoogleSans-Medium', marginTop:15, color:'black'}}>CONFIRMATION OF INSPECTION</Text>
        ...
        <View style={styles.detailsView}>  
		 ...
		 <View style={{flexDirection:'row', justifyContent:'space-between'}}>
		  <View style={{width:'50%'}}>
		   ...
		   <Text style={{color:'black', flex:1,  flexWrap: 'wrap'}}>{this.state.vendorInfo.woGpNumber}</Text>    
		  </View>
		   <View style={{width:'50%'}}>
		    <Text style={{color:'black', fontFamily:'GoogleSans-Medium', textAlign:'right'}}>Material Category</Text>
		    <Text style={{color:'black', textAlign:'right', flex:1,  flexWrap: 'wrap'}}>{this.state.vendorInfo.materialName}</Text>  
		  </View> 
		 ...
		 <View style={{flexDirection:'row', justifyContent:'space-between', marginTop:15}}>
		  <View style={{width:'50%'}}>
		   ...
		   <Text style={{color:'black', flex:1,  flexWrap: 'wrap'}}>{this.state.vendorInfo.vendorWorkAddress}</Text>
		  </View>
		      
		  <View style={{width:'50%'}}>
    	   <Text style={{color:'black', fontFamily:'GoogleSans-Medium', textAlign:'right'}}>Work Order Date</Text>
		   <Text style={{color:'black', textAlign:'right', flex:1,  flexWrap: 'wrap'}}>{this.state.vendorInfo.nominatioDateTime}</Text>
		  </View> 
		 </View>  
		 ...
		 <View style={{flexDirection:'row', justifyContent:'space-between', marginTop:15}}>
		  <View style={{width:'50%'}}>
		   ...
		   <Text style={{color:'black', flex:1,  flexWrap: 'wrap'}}>{this.state.vendorInfo.materialScName}</Text>
		  </View>
		      
		  <View style={{width:'50%'}}>
		    <Text style={{color:'black', fontFamily:'GoogleSans-Medium', textAlign:'right'}}>Total Quantity</Text>
		    <Text style={{color:'black', textAlign:'right', flex:1,  flexWrap: 'wrap'}}>{this.state.vendorInfo.offerQty}</Text>  
		  </View> 
		 </View> 
		</View>   
		...
		<View style={{marginHorizontal:10, borderRadius:5, elevation:8, backgroundColor:'#ffffff', padding:15}}>
			...
			<View style={{flexDirection:'row', marginTop:10}}>
			  ...
			  <View  style={{width:'35%'}}>
			    <TouchableOpacity onPress={()=>{this.setState({isTimePickerVisible:true})}}  style={{ flexDirection: 'row' }}>
			  ...
			  <View style={{marginTop:10, borderWidth:1, borderColor:'#418bca'}}>
			    <TextInput      
				  ...
				  />     
				     ...
				     {   
                    (Platform.OS === 'ios') ?
				     	<TouchableOpacity style={{ height: 40, padding:10, borderWidth:1, borderColor:'#418bca', marginTop:10}} onPress={() => {
             ...
             <View style={{ marginVertical:10}}>
				  <TouchableOpacity  onPress={()=>this.onSubmit()}  style={{borderRadius: 5, backgroundColor: '#418bca'}}>
					<Text style={{fontSize:15, color:'#ffffff', fontFamily:'GoogleSans-Medium', paddingVertical:15, textAlign:'center' ,elevation:5}}>SUBMIT</Text>
				  </TouchableOpacity> 
             </View>
...
const styles = StyleSheet.create({
  container: {
	...
	detailsView:{
	  borderWidth:1, margin:10, padding:15, backgroundColor:'#FEC1A5', elevation:8, borderColor:'transparent'
	}
});  
