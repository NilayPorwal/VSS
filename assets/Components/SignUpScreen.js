...
import {Platform, SafeAreaView, Image, Modal, ImageBackground, TouchableOpacity, Dimensions, StyleSheet, 
        Text, View, AsyncStorage, TouchableNativeFeedback, TextInput, Button, Alert, ScrollView, KeyboardAvoidingView } from 'react-native';
...
export class SignUpScreen extends React.Component {
 ...
 onSignUp(){
  ...
  getOTP(){
      if(this.state.FirstName != null && this.state.Password != null && this.state.Email != null && this.state.Mobile != null){
    
      fetch( APIManager.host + 'public/sso/otp/generator?title=IOS_REG_OTP_MSG_EMAIL&type=e&receiver=' + this.state.Email,  {
        method: 'POST',    
      headers: {  
       'Content-Type': 'application/json',  
      }, 
  ...
  async onLogIn(){        
   ...
   if(this.state.signInPassword != null && this.state.signInMail != null){
    ...
    APIManager.signin(user, pass,                 
      (responseJson)=> {                      
         // alert(JSON.stringify(responseJson));          
     if(responseJson.valid == true) {
       this.setState({loginData:responseJson}, ()=>{this._storeLoginData() }) 
                 
       APIManager.getOTP(responseJson.mobile,
       (responseJson)=> {                      
        // alert(JSON.stringify(responseJson));
          this.setState({data:responseJson.data,isRefreshing:false,otpModalVisible:true })
       })                             
        ...
        this.setState({isRefreshing:false}, ()=>{Alert.alert('Invalid Username or Password')})
      }    
       })                                
           }      
      else{
  ...
  <ImageBackground source={require('../Images/background.png')} style={{width: '100%', flex:1}}>   
   <ScrollView style={{flex:1}} showsVerticalScrollIndicator={false}>
    ...
    <KeyboardAvoidingView style={{marginTop:15, width:'75%',  flexDirection:'row'}}>
      ...
      />
        <TouchableOpacity onPress={()=>this.setState({ securePassword: !this.state.securePassword })} style={{position:'absolute', right:0, paddingTop:10}}> 
     {(this.state.securePassword==true)? 
      ...
      <TouchableOpacity onPress={()=> this.getOTP() } style={styles.button}>
       ...
       <TouchableOpacity onPress={()=>this.props.navigation.goBack()} style={{position:"absolute", top:10, left:10}}>
      ...
      <View  style={{ flex: 1,  justifyContent: 'center', alignItems: 'center',backgroundColor:'#00000080'}}>
       <View style={{ width: '80%',backgroundColor:'#ffffff', borderRadius:10, justifyContent:'center', alignItems:'center', padding:15  }}>
            
      <View style={{marginTop:10}}>
        <Text style={styles.text}>Verify OTP</Text>
        <TextInput
           style={{marginTop:5,  borderBottomWidth:(Platform.OS === 'ios')?1:0, paddingVertical:(Platform.OS === 'ios')?10:10 }}
      ...
      <TouchableOpacity onPress={()=>this.setState({otpModalVisible:false})} style={{position:'absolute', top:0, right:5}}>
...
const styles = StyleSheet.create({
  ...
  button:{
     borderRadius: 5, backgroundColor: '#ff7f00', marginTop:15, width:'80%',height:50,  justifyContent:'center', alignItems:'center', elevation:8
  },          
  buttonText:{        
  ...
  otpButton:{
    borderRadius: 5, backgroundColor: '#ff7f00', marginTop:15
  }
