...
type Props = {};
export default class ActiveSiteOffers extends Component<Props> {
	constructor(props) {
		super(props);
		this.state = {
			isRefreshing: true,
	...
	_storeVendorData(item) {
		AsyncStorage.setItem('insId', item.inspectorAiId.toString());
		AsyncStorage.setItem('vndrId', item.vendorAiId.toString());
		AsyncStorage.setItem('woId', item.vendorWoAiId.toString());
		AsyncStorage.setItem('matId', item.materialAiId.toString());
		AsyncStorage.setItem('matscId', item.materialSubcategoryAiId.toString());
	}
