...
import {Platform, StyleSheet, Text, View, Image, TouchableOpacity, 
        ScrollView, Modal, TouchableWithoutFeedback, BackHandler, Linking,
		TextInput, ImageBackground, Picker, Alert, ToastAndroid, CameraRoll} from 'react-native';
...
type Props = {};
export default class Inspection extends Component<Props> {
	  constructor(props) {  
		 super(props); 
	...
	getTestStatus(){
		...
		APIManager.checkTestStatus(this.state.materialDetails.inspectionMatAiId, 
        (responseJson)=> {
         //alert(JSON.stringify(responseJson));
		 this.setState({uatCount:responseJson.data.uatCount, obsCount:responseJson.data.obsCount, gtpCount:responseJson.data.gtpCount})
		 })   
	...
	onPassTest(){
	  this.getTestStatus()	
	  this.setState({overAllTestStatus:2}, ()=>{this.checkTestStatus()})
    ...
    checkForTest(){
     ...
     onConfirmInsp(){
		...
		//alert(Details) 	    
  		APIManager.updateMatInspStatus(Details,  
        (responseJson)=> {
	       //alert(JSON.stringify(responseJson));
  ...
  downloadAprdDrawing(){  
	 ...
	 getDownloadLink(){
    	APIManager.getDownloadLink(this.state.materialDetails.woAiId,  
        (responseJson)=> {
		  //alert(JSON.stringify(responseJson));
	       if (responseJson.status == 'SUCCESS') { 
               this.setState({imageLink:responseJson.data.file}, ()=>{ this.downloadAprdDrawing()})
		   }
           else{
 ...
 render() {      
    return (    
     ...
     <View style={{width:'20%'}} style={{flexDirection:'row'}}>
      ...
      <View style={{width:'80%'}}> 
       <Text style={{fontSize:18, fontFamily:'GoogleSans-Medium', color:'black', paddingTop:15, textAlign:'center' }}>INSPECTION</Text>
      </View>  
     </View>  
      ...
      <View style={styles.container}>
        ...
        <View style={{borderWidth:0, borderRadius:5, padding:15, backgroundColor:'#FEC1A5', width:'90%',  elevation:8}}>
		     
	     <View style={{flexDirection:'row'}}>
		   <Text style={{color:'#000000', fontFamily:'GoogleSans-Medium',  width:'50%'}}>SITE OFFER ID</Text>
		   <Text style={{color:'#000000', flex:1,  flexWrap: 'wrap', textAlign:'right', width:'50%'}}>{this.state.siteOfferDetails.siteOfferUnqId}</Text>
		 </View>
		  
		  <View style={{flexDirection:'row', marginTop:20}}>
    	   <Text style={{color:'#000000', fontFamily:'GoogleSans-Medium', width:'50%'}}>Materials Name</Text>
		   <Text style={{color:'#000000', flex:1,  flexWrap: 'wrap', textAlign:'right', width:'50%'}}>{this.state.siteOfferDetails.materialName}</Text>
		  </View>  
	    ...
	    <View style={{flexDirection:'row',margin:15}}>
			<TouchableOpacity disabled={(this.state.uatCount!=0)?true:false} onPress={()=>this.reDirectTo('AcceptanceTest')}  style={{width:'50%', justifyContent:'center', alignItems:'center'}}>
			  <Image source={require('../Images/IspectionTestIcons/Acceptance-Test.png')} style={{ borderWidth:1, borderRadius:5}}  /> 
		   {(this.state.uatCount!=0)?  
              <Icon name="check-circle" size={25} color="#3CB043" style={{position:"absolute", top:0, right:10}} />:null} 
			</TouchableOpacity>                        
			    
			          
			<TouchableOpacity disabled={(this.state.gtpCount!=0)?true:false} onPress={()=>this.reDirectTo('GTPTestScreen')} style={{width:'50%', justifyContent:'center', alignItems:'center'}}>
			  <Image source={require('../Images/IspectionTestIcons/gtp-master.png')} style={{ borderWidth:1, borderRadius:5}}/>
		 ...
		 <View style={{flexDirection:'row', margin:15}}>
			<TouchableOpacity onPress={()=>this.getDownloadLink()}  style={{width:'50%', justifyContent:'center', alignItems:'center'}}>
			  <Image source={require('../Images/IspectionTestIcons/approved-drawing.png')} style={{ borderWidth:1, borderRadius:5}} /> 
			</TouchableOpacity>      
			        
			<TouchableOpacity disabled={(this.state.obsCount!=0)?true:false} onPress={()=>this.reDirectTo('ObservationsScreen')} style={{width:'50%', justifyContent:'center', alignItems:'center'}}>
			  <Image source={require('../Images/IspectionTestIcons/Observations.png')}  style={{ borderWidth:1, borderRadius:5}} />
		...
		<Modal  
          ...
          onRequestClose={() => {
            ...
            <TouchableOpacity onPress={()=>this.setState({testModalVisible:false})} style={{ flex: 1,  justifyContent: 'flex-end', alignItems: 'center',backgroundColor:'#00000080'}}>
			<TouchableWithoutFeedback>
			<View style={{ width: 300, height:300,backgroundColor:'#ffffff',borderRadius:10, marginBottom:20}}>
			 <View style={{backgroundColor:'black', padding:15}}>
			  <Text style={{fontFamily:'GoogleSans-Medium', color:'white', fontSize:18, textAlign:'center'}}>OVERALL TEST RESULT</Text>
			  {(this.state.overAllTestStatus == 2)?<Text style={{fontFamily:'GoogleSans-Medium', color:'green', fontSize:25, textAlign:'center'}}>MATERIAL PASSED</Text>:
			      <Text style={{fontFamily:'GoogleSans-Medium', color:'red', fontSize:25, textAlign:'center'}}>MATERIAL FAILED</Text>}
				  
			 </View>
			 
             <View style={{justifyContent:'center', padding:10, marginTop:20}}>			 
			  <Text style={{color:'black', textAlign:'center'}}>DO YOU WANT TO SUBMIT YOUR TEST REPORT FOR MATERIAL S.NO. {this.state.materialDetails.materialSrNo}?</Text> 
			 </View>
			  
			  <View style={{flexDirection:'row', justifyContent:'space-between', padding:15}}>
				<TouchableOpacity onPress={()=>this.onYes()} style={{ borderRadius: 5, backgroundColor: '#ff7f00', margin:15}}>
				  <Text style={styles.buttonText}>Yes</Text>
				</TouchableOpacity>    
				
				<TouchableOpacity onPress={()=>this.setState({testModalVisible:false})} style={{ borderRadius: 5, backgroundColor: '#ff7f00', margin:15}}>
		...
		<Modal    
          ...
          onRequestClose={() => {
            ...
            <TouchableOpacity onPress={()=>this.setState({testModalVisible2:false})} style={{ flex: 1,  justifyContent: 'flex-end', alignItems: 'center',backgroundColor:'#00000080'}}>
			<TouchableWithoutFeedback>
			<View style={{ width: 300, height:300,backgroundColor:'#ffffff',borderRadius:10, marginBottom:20, padding:15}}>
			    
			  <View style={{flexDirection:'row', marginTop:20}}>
			   <Text style={{color:'black', textAlign:'center', width:'80%', fontFamily:'GoogleSans-Medium',}}>Upload the form 9 duly signed by You and Vendor</Text>
			   <TouchableOpacity style={{padding:10, borderWidth:1, borderRadius:5, backgroundColor:'brown', width:'20%'}}>
				 <Icon name="file-text" size={30} color="white" />  
				</TouchableOpacity>   
              </View>	
             
			  <View style={{flexDirection:'row', marginTop:20}}>
			   <Text style={{color:'black', textAlign:'center', width:'40%', fontFamily:'GoogleSans-Medium',}}>Seal Start No.</Text>
			 ...
			 <View style={{flexDirection:'row', marginTop:20}}>
			   <Text style={{color:'black', textAlign:'center', width:'40%', fontFamily:'GoogleSans-Medium',}}>Seal End No.</Text>
...
const styles = StyleSheet.create({
  container: {
    ...
    button:{
     borderRadius: 5, backgroundColor: '#ff7f00',  marginTop:20,  marginBottom:50		
	},
	buttonText:{    
     fontSize:18, color:'#ffffff', fontFamily:'GoogleSans-Medium', textAlign:'center', padding:10    		
	},
	acceptButton:{
      borderRadius: 5, backgroundColor: '#3CB043',  width:'40%', margin:10		
	},
	
	rejectButton:{
     borderRadius: 5, backgroundColor: '#fb0102',  width:'40%',  margin:10			
	},      
