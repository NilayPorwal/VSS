...
type Props = {};  
export default class InspectionsInProcess extends Component<Props> {
 ...
 async getVendorList(){  
        const inspId = await this.state.inspId
     	APIManager.getVendorList(inspId,     
		  (responseJson)=> {
  ...
  async getInspDetails(){  
		 await AsyncStorage.getItem('InspId').then((value) => 
			this.setState({inspId:value},()=>{this.getVendorList()} )	
		 )          
	 }    
    ...
    return (
	  ...
	  <View style={styles.container}>
	   <Loader loading={this.state.isRefreshing} color="#40a7ab" />
        <Text style={{fontSize:18, fontFamily:'GoogleSans-Medium', marginTop:25, color:'black', textAlign:"center"}}>INSPECTIONS INPROCESS</Text>
        ...
        {this.state.vendorList.map((item, key)=>
         ...
         <View style={styles.cardStyle}>
          ...
          <View style={{flexDirection:'row',backgroundColor:'#141F25', padding:10}}>
           ...
           <Text  style={{color:'#ffffff', fontSize:15,fontFamily:'GoogleSans-Medium',  paddingLeft:5}}>Offer Id : </Text> 
           <Text style={{color:'#ffffff', fontFamily:'GoogleSans-Medium', fontSize:15}}>{item.pdiOfferUnqId}</Text>
          </View>
          ...
          <View style={{padding:10}}> 
           ...
           <View style={{flexDirection:'row', marginVertical:5}}>
            ...
            <Text style={{color:'#000000', fontSize:15, paddingLeft:5}}>{item.vendorWorksAddress}</Text>  
           </View> 
             
          </View>       
          <TouchableOpacity onPress={()=>this.reDirectTo(item) } style={{backgroundColor: '#ff7f00'}}>
			  <Text style={{fontSize:15, color:'#ffffff', fontFamily:'GoogleSans-Medium', paddingVertical:10, textAlign:'center',  elevation:5}}>INPROCESS</Text>
		   </TouchableOpacity> 
          ...
          onRequestClose={() => {
           ...
           <TouchableOpacity onPress={()=>this.setState({detailsModalVisible:false, workAddress:''})} style={{ flex: 1,  justifyContent: 'center', alignItems: 'center',backgroundColor:'#00000080'}}>
			...
			<View style={{marginTop:50}}> 
				<Text style={{color:'black', fontSize:15, textAlign:'center'}}>{global.InspectionsPerformed.state.workAddress}</Text>   
		    </View>
			</View>                         
