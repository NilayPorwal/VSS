...
import {Platform, StyleSheet, Text, View, Image, 
		...
		BackHandler, TouchableWithoutFeedback, Picker,ActivityIndicator,
...
type Props = {};
export default class SiteOffersDetails extends Component<Props> {
	...
	this.setState({siteOfferStatus:1, isLoading:true},()=>{this.uploadStatus()})
	} 
	
	
	 onReject(){
		
		this.setState({siteOfferStatus:2},()=>{this.uploadStatus()})
	 ...
	 const Details =	{
		 ...
		 APIManager.uploadStatus(Details, 
        (responseJson)=> {
		 if(responseJson.status == 'SUCCESS'){	
         // alert(JSON.stringify(responseJson));
		  if(this.state.siteOfferStatus==1){
			 ...
			 this.setState({testModalFail:false}, ()=>{this.props.navigation.push('HomeScreen')}) 
		  }		  
  ...
  getSiteOfferPDF(){   
	...
	APIManager.getSiteOfferPDF(this.state.siteOfferDetails.inspectionSiteAiId,
       (responseJson)=> {
		//alert(JSON.stringify(responseJson)); 
	    if(responseJson.status == 'SUCCESS'){
        this.setState({pdfLink: responseJson.data.file}, ()=>{this.downloadPDF()})
         }  else{
         	  this.setState({isRefreshing:false})
         	  ToastAndroid.showWithGravity('SiteOffer Not Found', ToastAndroid.LONG, ToastAndroid.CENTER);
         }     
  ...
  render() {    
	...
	<ScrollView>   
	 <View style={styles.container}>
	   <Text style={{fontSize:20, fontFamily:'GoogleSans-Medium', marginTop:20, color:'black', textAlign:'center'}}>SITE OFFER DETAILS</Text>
	...
	{(this.state.error==false)?
	  <View>
		...
		<View style={styles.detailsView}>
         ...
         <Text style={{color:'black', fontFamily:'GoogleSans-Medium', marginLeft:15, marginTop:15, textAlign:'left'}}>Material Details</Text>  
		 ...
		 <View style={{flexDirection:'row'}}>
		  ...
		  <View style={{width:'50%',  alignItems:'flex-end'}}>
		   ...
		   <Text style={{color:'black',  flex:1,  flexWrap: 'wrap'}}>{this.state.siteOfferDetails.deliverySchUnit}</Text>
		  </View>
		 </View>   
		 ...
		 <View style={{flexDirection:'row',marginTop:20}}>
		  ...
		  <TouchableOpacity onPress={()=>this.getSiteOfferPDF()} style={{width:'50%'}}>
    	   <Text style={{ color:'#1b6379', textDecorationLine: 'underline', fontFamily:'GoogleSans-Medium', fontSize:15, textAlign:'right'}} >Download Site Offer</Text>
		  </TouchableOpacity> 
		 </View>   
		 ...
		 <View style={styles.tableContent}>
		  ...
		  <Text style={[styles.tableContentText,{width:'30%'}]}>{this.state.siteOfferDetails.deliverySchDate}</Text>
		  <Text style={[styles.tableContentText,{width:'30%'}]}>{this.state.siteOfferDetails.deliverySchQty}</Text>  
		 </View>  
		   
		 <View style={styles.tableContent}>  
		  <Text style={[styles.tableContentText,{width:'40%'}]}>PDI (Offer)</Text>
		  <Text style={[styles.tableContentText,{width:'30%'}]}>{this.state.siteOfferDetails.offerSupplyDat}</Text>
		  <Text style={[styles.tableContentText,{width:'30%'}]}>{this.state.siteOfferDetails.offerQty}</Text>
		 </View> 
		         
		 <View style={styles.tableContent}>
		  <Text style={[styles.tableContentText,{width:'40%'}]}>Site Offer</Text>
		  <Text style={[styles.tableContentText,{width:'30%'}]}>{this.state.siteOfferDetails.siteOfferDate}</Text>
		  <Text style={[styles.tableContentText,{width:'30%'}]}>{this.state.siteOfferDetails.inspectionSiteOfferQty}</Text>
		 </View>   
		</View> 
		</View>  		              
	  ...
	  {(this.state.vendorDetails.siteOfferStatus == 0)?	    
	   <View style={{flexDirection:'row', justifyContent:'space-between', alignItems:'center', width:'90%', marginBottom:50, marginTop:20, marginLeft:15}}>
		...
		<TouchableOpacity onPress={()=>this.setState({testModalFail:true})} style={styles.rejectButton}>
      ...
      <Modal    
          ...
          onRequestClose={() => {
            ...
            <TouchableOpacity onPress={()=>this.setState({testModalFail:false})} style={{ flex: 1,  justifyContent: 'flex-end', alignItems: 'center',backgroundColor:'#00000080'}}>
			<TouchableWithoutFeedback>
			<View style={{ width: 300, height:270,backgroundColor:'#ffffff',borderRadius:10, marginBottom:20, padding:15, justifyContent:'center'}}>
			  <Text style={{fontSize:15, fontFamily:'GoogleSans-Medium', color:'#000000', textAlign:'center'}}>REJECTION REASON</Text>   
	         ...
	         <View style={{ width:'100%', paddingHorizontal:10, marginTop:15}}>
				...
				<View style={{alignItems:'center'}}>
					<TouchableOpacity onPress={()=>this.onReject()} style={{borderRadius: 5, backgroundColor: '#3CB043', margin:10, paddingHorizontal:15, paddingVertical:10}}>
...
const styles = StyleSheet.create({
  container: {
	...
	acceptButton:{
     borderRadius:5,  backgroundColor: '#3CB043',  width:'30%', paddingVertical:10, alignItems:'center'		
	},
	rejectButton:{
     borderRadius:5,  backgroundColor: '#FF2300',  width:'30%', paddingVertical:10, alignItems:'center'		
	},
	buttonText:{
