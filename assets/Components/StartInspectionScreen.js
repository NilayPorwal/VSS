...
import {Platform, StyleSheet, Text, View, Image, TouchableOpacity, 
        AsyncStorage,ImageBackground, ActivityIndicator, ScrollView, 
		PermissionsAndroid ,BackHandler} from 'react-native';
 ...
 async calDistance(position){
  ...
  _distPerc(){     
   ...
   onStartInsp(){
     ...
     APIManager.uploadLatLong(Details,  
	  (responseData) =>{ 
  ...
  componentWillUnmount = () => {  
   ...
   onLogOut(){
	...
	async removeStoreData(){
		...
		await AsyncStorage.removeItem('InspName')
  ...
  render() {
    return (
      ...
      <View style={{width:'80%'}}> 
       <Text style={{fontSize:18, fontFamily:'GoogleSans-Medium', color:'black', paddingTop:15, textAlign:'center' }}>SITE OFFER</Text>
      </View>  
     </View> 
       
      <View style={styles.container}>
	   <Text style={{fontSize:18, fontFamily:'GoogleSans-Medium', color:'#ff7f00', marginTop:10, textAlign:'center'}}>Mapping Geolocation With Work Address</Text>
	     ...
	     <View style={{marginTop:50}}>
		  ...
		  {(this.state.atLocation==0)?
			 <View style={{alignItems:'center'}}>  
			   ...
			   <Text style={{fontSize:20, color:'green', fontFamily:'GoogleSans-Medium', marginTop:10}}>You Are At Site</Text>
			  </View>:
			  <View style={{alignItems:'center'}}>  
			   <Text style={{fontSize:20, color:'red', fontFamily:'GoogleSans-Medium', marginTop:10}}>You Are Not At Site</Text>
	         </View>}
