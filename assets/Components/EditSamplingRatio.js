...
import {Platform, StyleSheet, Text, View, Image, TouchableOpacity,Alert,KeyboardAvoidingView,
        ScrollView,ActivityIndicator, ImageBackground, BackHandler,ActionSheetIOS,
		AsyncStorage,Picker, TextInput, FlatList, Modal, Dimensions, Linking} from 'react-native';
...
type Props = {};
export default class EditSamplingRatio extends Component<Props> {
	...
	handleBackPress(){
	 ...
	 getSamplingList(){
        ...
        console.log(JSON.stringify(data))
    	 APIManager.getSamplingList(data, (responseJson)=> {
    ...
    onSave(){
    	...
    	if(this.state.samplingDataNew.length > 0 && this.state.samplingDataNew[this.state.samplingDataNew.length-1].qtyEndNo > this.state.startNo){
        ...
        for(let i=0; i<this.state.samplingData.length; i++ ){
           if((this.state.startNo == this.state.samplingData[i].qtyEndNo || this.state.startNo < this.state.samplingData[i].qtyEndNo) && (this.state.startNo == this.state.samplingData[i].qtyStartNo || this.state.startNo > this.state.samplingData[i].qtyStartNo)){
             return Alert.alert("Wait", "Incorrect Quantity Start No.")
           }
        }

        for(let j=0; j<this.state.samplingDataNew.length; j++ ){
           if((this.state.startNo == this.state.samplingDataNew[j].qtyEndNo || this.state.startNo < this.state.samplingDataNew[j].qtyEndNo) && (this.state.startNo == this.state.samplingDataNew[j].qtyStartNo || this.state.startNo > this.state.samplingDataNew[j].qtyStartNo)){
               return Alert.alert("Wait", "Incorrect Quantity Start No.")
        ...
        //alert(JSON.stringify(data))
      	this.setState({modalVisible:false, samplingDataNew:[...this.state.samplingDataNew, data], startNo:null, endNo:null, sampleTaken:null, defectQty:"0", noPoles:"0"})
    } 

    onSaveSample(){
    	this.setState({error:true, isLoading:true})  

    	 APIManager.onSaveSample(this.state.samplingDataNew, (responseJson)=> {
    	    this.setState({isLoading:false})  
    ...
    onUpdateSampleQty(index){
    	if(this.state.samplingData[index].qtySample > this.state.samplingData[index].qtyEndNo || this.state.samplingData[index].qtySample < this.state.samplingData[index].qtyStartNo){
        ...
        const data = [this.state.samplingData[index]]
    	 ...
    	 APIManager.onSaveSample(data, (responseJson)=> {
    	    this.setState({isLoading:false})  
	        if (responseJson.status == 'SUCCESS'){ 
              Alert.alert("SAVED", "Data saved successfully")
  ...
  render() {
    return (
     <ImageBackground source={require('../Images/background.png')} style={{width: '100%', height: '100%'}}> 
      ...
      <View style={{width:'80%'}}> 
       <Text style={{fontSize:18, fontFamily:'GoogleSans-Medium', color:'black', paddingTop:15, textAlign:'center' }}>Edit Sampling Ratio</Text>
      </View>  
     </View> 

      <Text style={{fontSize:18, fontFamily:'GoogleSans-Medium', color:'black', paddingTop:15, textAlign:'center' }}>Existing Sampling List</Text>
	  ...
	  <FlatList    
       ...
       <View style={[{backgroundColor:'#FEC1A5',}, styles.detailsView]}>  
		 ...
		 <View style={{width:'50%'}}>
		  ...
		  <View style={{width:'50%'}}>
    	   <Text style={{color:'black', fontFamily:'GoogleSans-Medium', textAlign:'right'}}>Quantity End No.</Text>
		   <Text style={{color:'black', textAlign:'right', flex:1,  flexWrap: 'wrap'}}>{item.qtyEndNo}</Text>
		  </View>
		 </View>     
		 ...
		 <View style={{flexDirection:'row', justifyContent:'space-between', marginTop:15}}>
		  <View style={{width:'50%'}}>
    	   <Text style={{color:'black', fontFamily:'GoogleSans-Medium',}}>Qunatity Sample</Text>
		     <TextInput
				 style={{ width:'70%',  backgroundColor:'#ffffff', padding:5, borderRadius:5, marginTop:3}}
		 ...
		 <View style={{width:'50%'}}>
    	   <Text style={{color:'black', fontFamily:'GoogleSans-Medium', textAlign:'right'}}>Quantity Permit Defect</Text>
		   <Text style={{color:'black', textAlign:'right', flex:1,  flexWrap: 'wrap'}}>{item.qtyPermitDefect}</Text>
		  </View> 
		 </View>
       	 ...
       	 <View style={{flexDirection:'row', justifyContent:'space-between', marginTop:15}}>
			 ...
			 <View style={{width:'60%'}}>
	    	   <Text style={{color:'black', fontFamily:'GoogleSans-Medium'}}>No of Poles for transverse strength test</Text>
     ...
     <Text style={{fontSize:18, fontFamily:'GoogleSans-Medium', color:'black', paddingTop:15, textAlign:'center' }}>Add New Sampling</Text>
	...
	{(this.state.samplingDataNew.length > 0)?
	  <FlatList    
       ...
       <View style={[{backgroundColor:'#ffffff',}, styles.detailsView]}>  
		 ...
		 <View style={{width:'50%'}}>
    	   <Text style={{color:'black', fontFamily:'GoogleSans-Medium', textAlign:'right'}}>Permissible Defect Qty.</Text>
		   <Text style={{color:'black', textAlign:'right', flex:1,  flexWrap: 'wrap'}}>{item.qtyPermitDefect}</Text>
		  </View> 
		 </View>
		...
		{(this.state.samplingDataNew.length == 0)?	
			<View style={{borderRadius:5, backgroundColor:'#ffffff', padding:15, elevation:8, margin:15}}>
              <View style={{flexDirection:'row', marginTop:10}}>
			   <Text style={{color:'black', width:'60%', fontFamily:'GoogleSans-Medium', paddingTop:10}}>Quantity Start No. :</Text>
			   	  <TextInput 
            ...
            <View style={{flexDirection:'row', marginTop:15}}>
			   <Text style={{color:'black',width:'60%', fontFamily:'GoogleSans-Medium', paddingTop:10}}>Quantity End No. :</Text>
			   	 <TextInput
					 style={{ width:'40%', borderBottomWidth:1, borderColor:'#ff7f00', padding:5}}
            ...
            <View style={{flexDirection:'row', marginTop:15}}>
			   <Text style={{color:'black', width:'60%', fontFamily:'GoogleSans-Medium', paddingTop:10}}>Sample To Be Taken :</Text>
			   ...
			   <Text style={{color:'black',  width:'60%', fontFamily:'GoogleSans-Medium', paddingTop:10}}>Permissible Defect Qty. :</Text>
			 ...
			 <View style={{flexDirection:'row', marginTop:20}}>
			   <Text style={{color:'black',  width:'60%', fontFamily:'GoogleSans-Medium', paddingTop:10}}>No of Poles for transverse strength test :</Text>
         ...
         {(this.state.samplingDataNew.length > 0)?
             <View style={{flexDirection:'row'}}>
	             ...
	             <View style={{alignItems:'center', width:'50%'}}>
	               <TouchableOpacity onPress={()=>this.setState({modalVisible:true})} style={{borderRadius: 5, backgroundColor: '#418bca', marginVertical:20, paddingVertical:10, paddingHorizontal:20}}>
						<Text style={styles.buttonText}>Add More</Text>
					 </TouchableOpacity>		  
				   </View>

	              <View style={{alignItems:'center', width:'50%'}}>
	                 <TouchableOpacity onPress={()=>this.onSaveSample()}  style={{borderRadius: 5, backgroundColor: '#418bca', marginVertical:20, paddingVertical:10, paddingHorizontal:20}}>
...
const styles = StyleSheet.create({
  container: {
   ...
   detailsView:{
	  borderWidth:1, margin:10, padding:15,  elevation:8, borderColor:'transparent'
	}
});        
