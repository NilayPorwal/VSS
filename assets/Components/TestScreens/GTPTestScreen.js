...
import {Platform, StyleSheet, Text, View,BackHandler, Image, 
        TouchableOpacity, ScrollView,ImageBackground, Modal, ToastAndroid,
...
type Props = {};
export default class GTPTestScreen extends Component<Props> {
   ...
   getGTPList(){  
	    ...
	    APIManager.getGTPList(this.state.siteOfferDetails.materialAiId, this.state.siteOfferDetails.materialSubcategoryAiId, this.state.siteOfferDetails.vendorWoAiId,
        (responseJson)=> {
   ...
   addGTPStatus(){ 
     ...
     APIManager.addGTPStatus(Details,  
        (responseJson)=> {
		if(responseJson.status=="SUCCESS"){	
		 ...
		 this.setState({gtpList:array, testModalVisible:false,isLoading:false, gtpStatus:null, gtpRemarks:'', gtpPhoto:'', imageData:null})
		}
		else{
	...
	onPassTest(item, index){
	  this.setState({gtpStatus:0,gtpAiId:item.gtpId, itemIndex:index, isLoading2:true}, ()=>{this.addGTPStatus()})
	   ToastAndroid.show('GTP Pass Successfuly !', ToastAndroid.CENTER); 
    }
	
	
	onGtpObs(item, index){
	  this.setState({testModalVisible:true, gtpStatus:0, gtpAiId:item.gtpId, itemIndex:index, gtpRemarks:item.gtpStandard})		
	}        
...
async on_PassAll(){ 
        ...
        let test ={"inspactMatAiId":this.state.materialDetails.inspectionMatAiId,
  ...
  async  _FailAll(){  
	 //await  this.on_FailAll()
		Alert.alert(
		  ...
		  [
			{text: 'NO', onPress: () => this.setState({gtpItems:[]}, ()=>{console.log('Cancel Pressed')}), style: 'cancel'},
...
render() {
    ...
    <View style={{width:'10%'}} style={{flexDirection:'row'}}>
      ...
      <View style={{width:'90%'}}> 
       <Text style={{fontSize:18, fontFamily:'GoogleSans-Medium', color:'black', paddingTop:15, textAlign:'center' }}>Test As Per GTP</Text>
      </View>  
     </View> 
	   ...
	   {(this.state.gtpList!='')?  
	    <View style={{flexDirection:'row'}}>
		   <View style={{width:"50%", paddingHorizontal:20}}>
			 <TouchableOpacity onPress={()=>this._PassAll()} style={{borderRadius: 5, backgroundColor: '#3CB043', paddingHorizontal:15}}>
			  <Text style={{fontSize:15, color:'#ffffff', fontFamily:'GoogleSans-Medium', paddingVertical:10, textAlign:'center'}}>PASS</Text>
			 </TouchableOpacity> 
		   </View>
		    <View style={{width:"50%", paddingHorizontal:20}}>
			 <TouchableOpacity onPress={()=>this._FailAll()} style={{borderRadius: 5, backgroundColor: 'red', paddingHorizontal:15}}>
			  <Text style={{fontSize:15, color:'#ffffff', fontFamily:'GoogleSans-Medium', paddingVertical:10, textAlign:'center'}}>FAIL</Text>
			 </TouchableOpacity> 
	         </View> 
	   </View>:null} 
          ...
          {(this.state.isRefreshing==false)?  
           <FlatList    
			...
			<View style={{margin:15, borderRadius:5, elevation:8, backgroundColor:'#ffffff', padding:15}}>
			  ...
			  <View style={{flexDirection:'row'}}>
			    <Text style={{color:'#000000', fontFamily:'GoogleSans-Medium', fontSize:15, width:'50%'}}>GTP Particulars</Text>
			    <Text style={{color:'#000000',  fontSize:15, flex:1,  flexWrap: 'wrap', textAlign:'right', width:'50%'}}>{item.gtpPartculars}</Text>
			  </View>  
			    
			  <View  style={{flexDirection:'row', marginTop:10}}>
			    <Text  style={{color:'#000000', fontFamily:'GoogleSans-Medium', fontSize:15, width:'50%'}}>GTP Unit</Text>
			    <Text  style={{color:'#000000', fontSize:15,flex:1,  flexWrap: 'wrap', textAlign:'right', width:'50%'}}>{item.gtpUnit}</Text>
			  </View> 
 
			   <View  style={{flexDirection:'row', marginTop:10}}>
			    <Text  style={{color:'#000000', fontFamily:'GoogleSans-Medium', fontSize:15, width:'50%'}}>GTP Standard</Text>
			    <Text  style={{color:'#000000', fontSize:15, flex:1,  flexWrap: 'wrap', textAlign:'right', width:'50%'}}>{item.gtpStandard}</Text>
			  </View> 
			   ...
			   <View  style={{flexDirection:'row'}}>
			    ...
			    <View style={{width:'50%'}}>
                   <TouchableOpacity  onPress={()=>this.onGtpObs(item, index)}  style={{borderRadius: 5, backgroundColor: '#418bca', marginTop:10}}>
					<Text style={{fontSize:15, color:'#ffffff', fontFamily:'GoogleSans-Medium', paddingVertical:10, textAlign:'center',  elevation:5}}>GTP Observation</Text>
				  </TouchableOpacity> 
			    </View>
       ...
       <Modal  
          ...
          onRequestClose={() => {
            ...
            <View onPress={()=>this.setState({testModalVisible:false})} style={{ flex: 1,  justifyContent: 'center', alignItems: 'center',backgroundColor:'#00000080'}}>
			
			<View style={{ width: 300, backgroundColor:'#ffffff',padding:10,borderRadius:10,justifyContent:'center',}}>
		{(this.state.isLoading==false)?
			 <View> 
			 {(this.state.imageData == null)? 	 
			 <View style={{flexDirection:'row', padding:10, marginTop:10, alignItems:'center'}}>
			  <Text style={{fontFamily:'GoogleSans-Medium',fontSize:15, color:'black'}}>Upload Documents (if any)</Text>
			   <TouchableOpacity onPress={()=>{this.onCamera()}} style={{padding:17, marginLeft:15, borderRadius:30, elevation:4}}>
				<Icon name="camera" size={30} color="black" />  
			   </TouchableOpacity>    
              </View>:  
			   <View style={{flexDirection:'row', padding:10, marginTop:10, alignItems:'center'}}>
			   <Text style={{fontFamily:'GoogleSans-Medium', fontSize:15, color:'black'}}>Document Uploaded</Text>
			 ...
			 <View style={{marginTop:10, alignItems:'center'}}>
		      ...
		      <View style={{alignItems:'center'}}>
			   ...
			   <TouchableOpacity onPress={()=>this.setState({testModalVisible:false, gtpRemarks:''})} style={{position:'absolute', top:0, right:5}}>
...
const styles = StyleSheet.create({
  container: {
    ...
    button:{
     borderRadius: 5, backgroundColor: '#ff7f00', 		
	},   
	buttonText:{
     fontSize:15, color:'#ffffff', fontFamily:'GoogleSans-Medium', paddingVertical:10, paddingHorizontal:25, 		
	},
	text:{
