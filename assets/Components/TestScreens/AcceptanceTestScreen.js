...
export default class AcceptanceTest extends Component {
	...
	onPassTest(){  
	 ...
	 if(this.state.testname !== ''){	
	  this.setState({test:[...this.state.test, { inspactMatAiId:inspectionMatAiId, uatTestName:this.state.testname,uatRemarks:this.state.observation,uatPhoto:this.state.imageData,photoExtension:this.state.type, uatVid:this.state.video, videoExtension:'mp4', uatStatus:"0" }]},
	...
	onFailTest(){ 
      const inspectionMatAiId = this.state.materialDetails.inspectionMatAiId	
	 if(this.state.testname !== ''){
	  this.setState({test:[...this.state.test, {inspactMatAiId:inspectionMatAiId,uatTestName:this.state.testname,uatRemarks:this.state.observation,uatPhoto:this.state.imageData,photoExtension:this.state.type,uatVid:this.state.video,  videoExtension:'mp4',uatStatus:"1"}]},		
      ()=>this.setState({testModalVisible:false, testname:'',imageData:null, observation:''}))	 
	 }
     else{
		alert('Please fill the Test Name') 
...
async onDoc(){
 ...
 uploadAcceptanceTest(){  
	...
	this.setState({isDisabled:true, isRefreshing:true})
	 ...
	 APIManager.uploadAcceptanceTest(Details, 
      (responseJson)=> {
  ...
  render() {
    return ( 
     ...
     <View style={{width:'20%'}} style={{flexDirection:'row'}}>
      ...
      <View style={{width:'80%'}}> 
       <Text style={{fontSize:18, fontFamily:'GoogleSans-Medium', color:'black', paddingTop:15, textAlign:'center' }}>Acceptance Test</Text>
      </View>  
     </View> 
     ...
     <View style={styles.container}>
       ...
       <View style={{borderWidth:1, margin:15}}>      
	    ...
	    this.state.test.map((item)=>  		     
		 <View style={styles.tableContent}>
		  <Text style={[styles.tableContentText,{width:'50%'}]}>{item.uatTestName}</Text>
		   {(item.uatStatus=="0")? <Text style={[styles.tableContentText,{width:'50%'}]}>PASS</Text>:
		...
		<TouchableOpacity onPress={()=>this.onAddTest()} style={{ borderRadius: 25, backgroundColor: '#ff7f00', padding:15, elevation:8, }}>
       ...
       <Modal      
          ...
          onRequestClose={() => this.onModalClose()}>
            <TouchableOpacity onPress={()=>this.onModalClose()} style={{ flex: 1,  justifyContent: 'center', alignItems: 'center',backgroundColor:'#00000080'}}>
			...
			<View style={{ width: 300, height:300,backgroundColor:'#ffffff',padding:10,borderRadius:10,}}>
			 ...
			 <View style={{flexDirection:'row'}}>	               
				...
				<View style={{flexDirection:"row", marginTop:10, justifyContent:'center', alignItems:'center', width:"30%"}}>	
				...
				<TouchableOpacity onPress={()=>this.removeDoc()}  style={{width:'30%',marginTop:10, justifyContent:'center', alignItems:'center'}}>
			...
			<TouchableOpacity onPress={()=>this.onModalClose()} style={{position:'absolute', top:0, right:5}}>
